...
TrackName = 'Track8-RoboCop';

warning('This Track cannot be used to calculate the flight error. Lane is not like a function.', 'No error calculation')
%% Track 1
trans.x.T1 = -0.3555; %B1 x
trans.y.T1 = 0.3516; %B1 y
lung.T1 = 1;
angle.T1 = deg2rad(135.3093); %alpha 1

%% Track 2
trans.x.T2 = -0.2452; %B2 x
trans.y.T2 = 0.8852; %B2 y
lung.T2 = 1;
angle.T2 = deg2rad(21.3346); %alpha 2

%% Track 3
trans.x.T3 = 0.364; %B3 x
trans.y.T3 = 1.5461; %B3 y
lung.T3 = 1;
angle.T3 = deg2rad(73.325); %alpha 3

%% Track 4
trans.x.T4 = 0.043; %B4 x
trans.y.T4 = 1.84; %B4 y
lung.T4 = 1;
angle.T4 = deg2rad(201.7211); %alpha 4

%% Circle
trans.x.C = -0.6537; %C x
trans.y.C = 1.5624; %C y
